...
bool AIMgr::DoAction()
{
	...
	*
	*	如果是逃脱策略（设定此策略，需要与失败条件绑定！即敌方到达此位置后，玩家即失败）：
	*	A、向目的地前进，如果在将要到达目的地时发现目的地被敌方占领，则攻击之，如果被友方占领，则行动至目的地旁，并通知友方离开
	*	B、到达目的地后，原地待命至系统判定玩家失败
	*
	*	如果是获取目标策略
	*	A、向目标前进，如果目标是据点型且被占领：占领者是敌方则攻击；占领者是友方则行动至目的地旁通知其离开；
	*	如果目标是宝箱且已经消失(被其他单位获取)，则根据胜利条件来判断接下来如何：
