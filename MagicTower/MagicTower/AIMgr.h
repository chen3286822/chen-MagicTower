...
public:
	...
	int GetDamageToTarget(Character* target,int& attack);	//返回对目标造成的最高预期伤害,attack是-1则表示是普通攻击，其他值表示是技能伤害
	bool CanKillTarget(Character* target,int& attack);	//是否可以击杀单位，attack是-1则表示是普通攻击，其他值表示是技能伤害
private:
	Character* m_pCurAI;
